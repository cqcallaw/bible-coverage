...
def test_whole_psalm() -> None:
    result = parser.parse("Psalm 100", nasb_bible)
    assert len(result) == 1
    assert result[0].book == "Psalm"
    assert result[0].chapter == 100
    assert not result[0].start_verse
